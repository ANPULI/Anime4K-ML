    ...
    for item in dirs:
        ...
        if os.path.isfile(path+item):
            ...
            if (im_height < orig_height):
                ...
                imResize = im.resize((im_width,im_height), Image.NEAREST)
            # print(output_path + item + ' resized.png')
            # break
            print(output_path + item.split('.')[0] + '_nn_resized_' + str(res) + '.png')
            imResize.save(output_path + item.split('.')[0] + '_nn_resized_' + str(res) + '.png', 'png', quality=90)
